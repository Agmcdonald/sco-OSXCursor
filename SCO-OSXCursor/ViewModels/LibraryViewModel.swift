...
// Import readers and metadata parser
import AppKit // For security-scoped bookmarks
...
final class LibraryViewModel: ObservableObject {
    ...
    init(database: DatabaseManager) {
        ...
        // restoreReadingProgress() // optional at startup
    }
    ...
    func importComics(from urls: [URL]) async {
        ...
        for (index, url) in urls.enumerated() {
            do {
                ...
                // Use a hash-based approach to create a deterministic UUID from the file path
                let normalizedPath = url.standardizedFileURL.path
                let pathData = normalizedPath.data(using: .utf8) ?? Data()
